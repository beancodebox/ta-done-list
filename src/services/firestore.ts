...
export async function syncWithCloud(
  ...
  if (localUpdatedAt === 0 || cloudData.timestamp > localUpdatedAt) {
    // 클라우드가 더 최신 → 클라우드 데이터 반환
    return { items: cloudData.itemList, timestamp: cloudData.timestamp }
  } else {
    // 로컬이 더 최신 → 로컬 데이터 저장
    const now = Date.now()
    ...
    return { items: localItems, timestamp: now }
  }
}
